...
type DayPlan = {
  ...
  note?: string;
  subject?: string; // For exams/reviews
};

const PLAN: DayPlan[] = [
  // Phase 1: Syllabus Completion (Dec 8 - Dec 15)
  ...
  const calculateSubjectProgress = (subjectCode: string) => {
    ...
    PLAN.forEach(day => {
      day.tasks?.forEach(task => {
        if (task.subjectCode === subjectCode) {
          if (completedTasks.has(getCompletionKey(day.date, task.subjectCode, task.unitId))) {
            completed++;
          }
        }
      });
